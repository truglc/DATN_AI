...
# ======================
def process_video(video_path):
    global frames, frame_count, label, confidence, predict_counter
    ...
    while cap.isOpened():
        ...
        # 🔥 resize nhỏ lại để tăng tốc
        frame = cv2.resize(frame, (640, 360))

        frame_count += 1

        # 🔥 skip frame để giảm load
        if frame_count % 5 == 0:
            ...
            if len(frames) == sequence_length:
                ...
                # 🔥 chỉ predict mỗi vài lần
                if predict_counter % 5 == 0:
                    input_data = np.expand_dims(frames, axis=0)
